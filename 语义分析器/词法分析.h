...
int Lexical_Analysis::ScanCode(record *opstream)
{
	InputBuffer(1);
	readptr = buffer1;
	char c;
			...
			if (i == MaxSizeOfBuffer) {
				...
				if (state != ER&&state != SKIP) {
					if (state == ID) {
						...
						opstream[ptr].type = "int";
						opstream[ptr].num = (float)transfer(tempNum, len);
						ptr++;
						RealNum++;
					}
					else if (state == OP1) {
